...
class LatesExit extends StatelessWidget {
  final String userID;
  LatesExit(this.userID);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: LatesExitBody(userID),
      ),
    );
  }
}

class LatesExitBody extends StatefulWidget {
  final String userID;
  LatesExitBody(this.userID);

  @override
  _LatesExitBodyState createState() => _LatesExitBodyState(userID);
}

class _LatesExitBodyState extends State<LatesExitBody> {
  String userID;
  _LatesExitBodyState(this.userID);
  ...
  _initialize() async {
    ...
    setState(() {
      ...
      FirebaseFirestore.instance.collection(myCompanyDbRegisters).where('process', isEqualTo: 'out').where('userID', isEqualTo: userID).get().then((value) {
        value.docs.forEach((document) {
          Timestamp time = document.get('time');
          var date = DateTime.fromMillisecondsSinceEpoch(time.millisecondsSinceEpoch);

          FirebaseFirestore.instance.collection(myCompanyDbRegisters).where('userID', isEqualTo: userID).where('date', isEqualTo: document.get('date')).where('process', isEqualTo: 'out').where('excuse_saved', isEqualTo: true).get().then((value) {
